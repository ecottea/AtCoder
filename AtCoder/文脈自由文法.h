...
/*
* 出発記号を 0，変数を [0..n)，終端記号を (-m..0) で表すものとする．
* 生成規則の本体は記号を順に並べたリストと重みの対 pair<vi, ll> で表し，
...
//【重み付きチョムスキー標準形への変換】O(|cfg|^2)
/*
* 重み付き文脈自由文法 cfg を重み付きチョムスキー標準形に変換したものを cnf に格納する．
* すなわち cnf の本体は「変数 2 つ」または「終端記号 1 つ」からなり，無用な記号をもたない．
*
...
//【重み付きチョムスキー標準形での導出コスト】O(n^3 |cnf|)
/*
* 重み付きチョムスキー標準形 cnf で語 w[0..n) を導出するのにかかる最小コストを返す（不可能なら INFL）
*
* 制約：cnf はチョムスキー標準形である．
*/
