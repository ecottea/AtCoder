...
template <class T> struct FPS {
	...
	FPS() : n(T::o()) {}
	FPS(const T& c0) : n(1), c({ c0 }) {}
	FPS(const T& c0, int d) : n(d), c(n) { c[0] = c0; }
	FPS(const vector<T>& c_) : n(sz(c_)), c(c_) {}

	// 代入
	FPS(const FPS& f) = default;
	FPS& operator=(const FPS& f) = default;
	FPS& operator=(const T& c0) { n = 1; c = { c0 }; return *this; }
	...
	// 定数加算
	FPS& operator+=(const T& sc) {
		if (n == 0) { n = 1; c = { sc }; }
		else { c[0] += sc; }
		return *this;
	}
	FPS operator+(const T& sc) const { return FPS(*this) += sc; }
	friend FPS operator+(const T& sc, const FPS& f) { return f + sc; }
	...
	// 定数減算
	FPS& operator-=(const T& sc) { *this += -sc; return *this; }
	FPS operator-(const T& sc) const { return FPS(*this) -= sc; }
	friend FPS operator-(const T& sc, const FPS& f) { return -(f - sc); }

	// 加法逆元
	FPS operator-() const { return FPS(*this) *= -T(T::e()); }

	// 定数倍
	FPS& operator*=(const T& sc) { rep(i, n) c[i] *= sc; return *this; }
	FPS operator*(const T& sc) const { return FPS(*this) *= sc; }
	friend FPS operator*(const T& sc, const FPS& f) { return f * sc; }

	// 積
	FPS& operator*=(const FPS<T>& g) {
		int m = g.deg();
		resize(n + m);
	...
	FPS operator*(const FPS<T>& g) const { return FPS(*this) *= g; }
	...
	FPS& operator/=(const FPS<T>& g) { return *this *= g.inv(n); }
	FPS operator/(const FPS<T>& g) const { return FPS(*this) /= g; }

	// 余り付き除算
	FPS quotient(const FPS<T>& g) const {
		// 参考 : https://nyaannyaan.github.io/library/fps/formal-power-series.hpp
	...
	FPS reminder(const FPS<T>& g) const { return (*this - this->quotient(g) * g).resize(g.n - 1); }
	pair<FPS, FPS> quotient_remainder(const FPS<T>& g) const {
		pair<FPS, FPS> res;
	...
	FPS& operator/=(const SFPS& g) {
		// g の定数項だけ例外処理
		auto it0 = g.begin();
		assert(it0->first == 0 && it0->second == T::e());
		it0++;
...
*/
template <class T>
T linearly_recurrent_sequence(const vector<T>& a, const vector<T>& c, ll n) {
	int d = sz(a);
