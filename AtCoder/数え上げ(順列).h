...
/*
* 各 i∈[0..n], j∈[0..m] について，
* 順列 p[0..i) で j 個の巡回置換の積で表されるものの個数を c[i][j] に格納する．
*/
void stirling_S1(int n, int m, vvm& c) {
	//【方法】
	// c[i][j] = S(i, j)（第 1 種スターリング数）であり，これは上昇階乗冪の展開係数
	//		S(i, j) = [x^j] x(x+1)(x+2)...(x+(i-1))
	// として求められる．ここから漸化式
	...
	// c[i][j] : [x^j] x(x+1)(x+2)...(x+(i-1))
	c = vvm(n + 1, vm(m + 1));
	c[0][0] = 1;

	repi(i, 1, n) repi(j, 1, m) c[i][j] = c[i - 1][j] * (i - 1) + c[i - 1][j - 1];
}


//【貪欲増加部分列長が指定された順列の数え上げ】O(n^3)
/*
...
*（挿入 DP）
*/
void count_permutations_greedy_increasing_subsequence(int n, vvm& cnt) {
