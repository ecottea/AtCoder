...
*	f を g で割った余りを返す．
*
* quotient_remainder(f, g) : O(n log n)
*	f を g で割った商と余りの組を返す．
*
* f.resize() : O(n)
*	不要な高次の項を削る．
...
struct FPS {
	...
	FPS operator+(const FPS& g) const { FPS h = *this; h += g; return h; }
	...
	FPS operator+(const mint& sc) const { FPS h = *this; h += sc; return h; }
	friend FPS operator+(const mint& sc, const FPS& f) { return f + sc; }
	FPS& operator+=(const int& sc) { *this += mint(sc); return *this; }
	FPS operator+(const int& sc) const { FPS h = *this; h += sc; return h; }
	...
	// 加法逆元
	FPS operator-() { FPS h = *this; h *= -1; return h; }
	...
	// 余り
	FPS operator%(const FPS& g) const { return (*this - (*this / g) * g).resize(g.n - 1); }
	FPS& operator%=(const FPS& g) { return *this = *this % g; }

	// 商と余り
	friend pair<FPS, FPS> quotient_remainder(const FPS& f, const FPS& g) {
		pair<FPS, FPS> res;
		res.first = f / g;
		res.second = (f - res.first * g).resize(g.n - 1);
		return res;
	}
