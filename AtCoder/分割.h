...
*		   条件 (x[i]=x[j] ⇔ i,j が同じブロックに属する) に変更したときの対象の個数
...
* 制約：fm は N! まで計算可能
*/
mint partition_subzeta_block_count(const vm& a, const Factorial_mint& fm) {
	//【方法】
	// 第 2 種スターリング数の一般項は
	//		s2(n,k)
...
* 制約：fm は N! まで計算可能
*/
mint partition_submobius_block_count(const vm& b, const Factorial_mint& fm) {
	//【方法】
	// 第 2 種スターリング数の一般項は
	//		s2(n,k)
	//		= (1/k!) Σm=[1..k] (-1)^(k-m) bin(k, m) m^n
...
* [0..N) 上の分割関数 a[π] の上位分割からの累積和が
*		b[π] = Σπ:ρの細分 a[ρ] （π:[0..N) の分割）
* であるとし，与えられた b に対応する a を返す．具体的には
...
* 利用：【形式的冪級数】，【指数関数】
*/
mint partition_supermobius_block(const vm& b, const Factorial_mint& fm) {
	// verify : https://atcoder.jp/contests/abc236/tasks/abc236_h
...
*/
template <class T>
T partition_supermobius_block(const vector<T>& b) {
	// verify : https://atcoder.jp/contests/abc236/tasks/abc236_h
...
*/
mint partition_supermobius_block_count(const vm& b, const Factorial_mint& fm) {
...
* 利用：【形式的冪級数】，【対数関数】
*/
mint partition_supermobius_block_size(const vm& b, const Factorial_mint& fm) {
	//【方法】
