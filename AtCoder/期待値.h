...
//【K 番目の期待値】
/*
* X[0..n) を [0..∞) を台とする確率変数，Kth を昇順 K 番目（1-indexed）とするとき，
*	E[Kth(X[0..n))]
*	= Σx∈[0..∞) P(X[0..n) のうち x 以下の値をとるものが K 個未満)
*	= Σx∈[0..∞) Σ|S|<K (Πi∈S P(X[i]≦x)) (Πi!∈S P(X[i]>x))		(X[0..n) が互いに独立のとき)
*	= Σx∈[0..∞) Σk∈[0..K) bin(n,k) P(X[i]≦x)^k P(X[i]>x)^(n-k)	(X[0..n) が i.i.d. のとき)
* 
*（証明）【しっぽ確率定理】を用いると，
*	E[Kth(X[0..n))]
*	= Σx∈[0..∞) P(Kth(X[0..n)) > x)
*	= Σx∈[0..∞) P(X[0..n) のうち x 以下の値をとるものが K 個未満)
* 
* verify : https://yukicoder.me/problems/no/3169
*/
...
*（停止確率に脱出までにかかるターン数の期待値を掛けて和をとれば良い）
* 
...
vector<T> sugoroku(int m, int n) {
	...
	//【備考】
	// dp[n+1] - dp[n] でマス n に止まる確率を計算できる．
	...
	// 貰う DP
	repi(i, 1, n) {
		dp[i] = 1 + (acc[i] - acc[max(i - m, 0)]) * inv_m;
		acc[i + 1] = acc[i] + dp[i];
	}
