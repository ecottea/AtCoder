...
//【線形加重和 モノイド】
/*
* S ∋ x = {xv, xs, xc} : x に対応する区間 x[0..n) についての以下の値を表す：
*	xv : 0,1,...,n-1 による x[0..n) の加重和
*	xs : 区間の総和
*	xc : 区間の長さ
*	(xv + xs      : 1,2,...,n による x[0..n) の加重和)
*	(xs * xc - xv : n,...,2,1 による x[0..n) の加重和)
* x op y : x, y に対応する区間を連結した区間を表す．
*/
...
*/
using T035 = int;
using S035 = pair<T035, T035>; // 行列 (a, b; 0, 1)（1 := ~0）
...
*/
using T036 = int;
using S036 = pair<T036, T036>; // 行列 (a, b; 0, 1)（1 := ~0）
S036 op036(S036 f, S036 g) {
	auto [a, b] = g; // 行列 (a, b; 0, 1)
	auto [c, d] = f; // 行列 (c, d; 0, 1)
...
//【最長連 モノイド】
/*
* S ∋ f = {fcL, flL, flA, fcR, flR, fl} : f に対応する文字列についての以下の値を表す：
*	fcL : 左端の文字
*	flL : 左端の文字を含む連の長さ
*	flA : 最長連の長さ
*	fcR : 右端の文字
*	flR : 右端の文字を含む連の長さ
*	fl  : 長さ
* f op g : f, g に対応する文字列をこの順に繋げた文字列を表す．
*/
